...
public class LocalDevice {
	 ...
	 * 
	 * @return the name of the local device; <code>null</code> if the name
	 ...
	 * device. This method will return <code>null</code> if the service
	 ...
	 *         of the local device, or <code>null</code> if the service
	 *         classes, major device class or minor device class could not be
	 *         determined
	 * 
	 */
	 ...
	 * Assigned Numbers Document. When this specification was defined, only GIAC (<code>DiscoveryAgent.GIAC</code>)
	 * and LIAC (<code>DiscoveryAgent.LIAC</code>) were defined, but
	 ...
	 * <code>DiscoveryAgent.GIAC</code> or <code>DiscoveryAgent.LIAC</code>
	 * are provided, then this method will attempt to put the device into
	 * general or limited discoverable mode, respectively. To take a device out
	 * of discoverable mode, provide the
	 * <code>DiscoveryAgent.NOT_DISCOVERABLE</code> flag. The BCC decides if
	 * the request will be granted. In addition to the BCC, the Bluetooth system
	 ...
	 *            <code>DiscoveryAgent.NOT_DISCOVERABLE</code> and any value
	 *            in the range 0x9E8B00 to 0x9E8B3F
	 * 
	 ...
	 * supported, but additional values are allowed: <TABLE>
	 * <TR>
	 * <TH>Property Name</TH>
	 * <TH>Description</TH>
	 * </TR>
	 ...
	 * <TD>The version of the Java API for Bluetooth wireless technology that
	 * is supported. For this version it will be set to "1.0".</TD>
	 * </TR>
	 * <TR>
	 ...
	 * <TD>The maximum number of connected devices supported. This number may
	 * be greater than 7 if the implementation handles parked connections. The
	 ...
	 * address will never be <code>null</code>. The Bluetooth address will be
	 * 12 characters long. Valid characters are 0-9 and A-F.
	 * 
	 ...
	 * <code>btl2cap</code>, or <code>btgoep</code> notifier. In the case
	 * of a run-before-connect service, the service record returned by
	 ...
	 *                <code>javax.bluetooth.L2CapConnectionNotifier</code>,
	 *                or <code>javax.obex.SessionNotifier</code>. This
	 *                exception is also thrown if <code>notifier</code> is not
	 *                a Bluetooth notifier, e.g., a
	 *                <code>StreamConnectionNotifier</code> created with a
	 ...
	 * <UL>
	 * <LI>ServiceClassIDList and ProtocolDescriptorList, the mandatory service
	 * attributes for a <code>btl2cap</code> service record, must be present
	 * in <code>srvRecord</code>.
	 * <LI>L2CAP must be in the ProtocolDescriptorList.
	 * <LI><code>srvRecord</code> must not have changed the PSM value from
	 * the PSM value that is currently in the SDDB version of this service
	 * record.
	 * </UL>
	 * 
	 * <P>
	 ...
	 * 
	 * @exception IllegalArgumentException
	 *                if the structure of the <code>srvRecord</code> is
	 *                missing any mandatory service attributes, or if an attempt
	 *                has been made to change any of the values described as
	 *                fixed.
	 * 
