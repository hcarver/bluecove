...
public interface AdapterV3 extends Adapter {
     ...
     * know the sting "Company ID %d" where %d should be replaced with the
    ...
    /**
     * Sets mode of the adapter. See GetMode for valid Strings for the mode
     * parameter.
     *
     * @param mode
     */
    ...
    /**
     * Returns the discoverable timeout in seconds. A value of zero means that
     * the timeout is disabled and it will stay in discoverable/limited mode
     * forever.
     *
    ...
    /**
     * Returns true if the local adapter is discoverable/limited and false if it
     * is only connectable or switched off.
     *
    ...
    /**
     * Returns the current major class value for this system. Currently, only
     * "computer" is supported. For the other values, unsupported major class
     * error is returned.
     */
    ...
    /**
     * Returns the current minor class value for this system where the default
     * major class is "computer".
     *
     * If the major class is not "computer" an error should be returned.
     *
     * Valid values: "uncategorized", "desktop", "server", "laptop", "handheld",
     * "palm", "wearable"
     *
     ...
     *
     * In the case no service classes are set (when no service has been
     * registered) an empty list should be returned.
     *
    ...
    /**
     * Get the remote device's name. This request returns always a cached name.
     * The service daemon is responsible for updating the cache.
     *
     * NotAvailable error is returned if the name is not in the cache. But if
     * there is a discovery running, then this function will return
     * RequestDeferred. In this case the service daemon will queue the request
     * and it will try to resolve the name at the next possible opportunity. On
     * success a RemoteNameUpdated signal will be send and if a failure happens
     * it will be indicated by a RemoteNameFailed signal.
     *
     * If this is an empty String, the UI might want to display the BD_ADDR
     * instead.
     *
     ...
     *
     * If a link key for this adapter already exists, this procedure should fail
     * instead of trying to create a new pairing.
     *
     * If no connection to the remote device exists, a low-level ACL connection
     * must be created.
     *
     * This function will block and the calling application should take care of
     * setting are higher timeout. This might be needed in case of a page
     * timeout from the low-level HCI commands.
     *
     ...
     *
     * If the link key was stored on the Bluetooth chip, it must be removed from
     * there, too.
     *
     * After deleting the link key this method will send a BondingRemoved
     * signal.
     */
    ...
    /**
     * Returns true if the remote device is bonded and false if no link key is
     * available.
     *
     * @param address
     * @return
    ...
    /**
     *
     This method will request the SDP database of a remote device and retrieve
     * the service record handles. To request service browse send an empty match
     * String.
     */
     ...
     * service record and return the binary stream of it.
     *
     */
