...
public class LocalDevice {
	 ...
	 * <code>DiscoveryAgent.LIAC</code>) for 1 minute. This is handled by the implementation of the API. After the
	 * minute is up, the device will revert back to the previous discoverable mode.
	 * 
	 ...
	 * <TD>Is paging allowed during a connection? In other words, can a connection be established to one device if it is
	 * already connected to another device. Valid values are either "true" or "false".</TD>
	 * </TR>
	 * </TABLE>
	 * 
	 ...
	 *                <code>javax.bluetooth.L2CapConnectionNotifier</code>, or <code>javax.obex.SessionNotifier</code>.
	 *                This exception is also thrown if <code>notifier</code> is not a Bluetooth notifier, e.g., a
	 ...
	 * Updating is possible only if <code>srvRecord</code> was obtained using the <code>getRecord()</code> method. The
	 * service record in the SDDB is modified to have the same service attributes with the same contents as
	 * <code>srvRecord</code>.
	 * 
	 * <p>
	 * 
	 ...
	 * <P>
	 * 
	 * If the <code>srvRecord</code> parameter is a <code>btspp</code> service record, then before the SDDB is changed
	 * the following checks are performed. If any of these checks fail, then an <code>IllegalArgumentException</code> is
	 * thrown.
	 * 
	 * <UL>
	 ...
	 * <LI><code>srvRecord</code> must not have changed the RFCOMM server channel number from the channel number that is
	 * currently in the SDDB version of this service record.
	 * </UL>
	 * 
	 * <P>
	 * 
	 * If the <code>srvRecord</code> parameter is a <code>btl2cap</code> service record, then before the SDDB is changed
	 * the following checks are performed. If any of these checks fail, then an <code>IllegalArgumentException</code> is
	 * thrown.
	 * <UL>
	 ...
	 * <LI><code>srvRecord</code> must not have changed the PSM value from the PSM value that is currently in the SDDB
	 * version of this service record.
	 * </UL>
	 * 
	 * <P>
	 * 
	 * If the <code>srvRecord</code> parameter is a <code>btgoep</code> service record, then before the SDDB is changed
	 * the following checks are performed. If any of these checks fail, then an <code>IllegalArgumentException</code> is
	 * thrown.
	 * <UL>
	 ...
	 * <p>
	 * 
	 * <code>updateRecord()</code> is not required to ensure that <code>srvRecord</code> is a completely valid service
	 * record. It is the responsibility of the application to ensure that <code>srvRecord</code> follows all of the
	 ...
	 * 
	 * @exception IllegalArgumentException
	 *                if the structure of the <code>srvRecord</code> is missing any mandatory service attributes, or if
	 *                an attempt has been made to change any of the values described as fixed.
	 * 
