...
IOBluetoothSDPDataElement* createTestDataElementSimple(jint type, jlong ldata, jbyte *bytes, int inBytesLen) {
...
IOBluetoothSDPDataElement* createTestDataElement(jint testType, jint type, jlong ldata, jbyte *bytes, int inBytesLen) {
    switch (testType) {
    case 0:
...
JNIEXPORT jbyteArray JNICALL Java_com_intel_bluetooth_NativeTestInterfaces_testOsXDataElementConversion
  (JNIEnv* env, jclass, jint testType, jint type, jlong ldata, jbyteArray bdata) {
    ...
    const IOBluetoothSDPDataElement* dataElement = createTestDataElement(testType, type, ldata, inBytes, inBytesLen);
