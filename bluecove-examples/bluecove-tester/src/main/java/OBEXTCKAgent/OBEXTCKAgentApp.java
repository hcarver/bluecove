...
public class OBEXTCKAgentApp extends Thread {
    ...
    final String BTGOEP_AGENT_SERVICE_SRVCLASS_ID
        = "4000000031b811d88698000874b33fc0";
    boolean cont;
    boolean AUTHENTICATE_TESTED=false;
    ...
    // the timeout set by the user.  If it is not set, use the default timeout value
    long timeout;
    
    // the transport protocol set by the user.  If it is not set, "btgoep" is the default
    String transport = "btgoep";
    

    /**
     * Creates a <code>OBEXTCKAgentApp</code> object.
     * @param timeoutVal the timeout value specified by the user
     * @param transportVal the transport protocol specified by the user
     */
    ...
    /**
     * Starts the <code>OBEXThread</code> which accepts OBEX
     * connections.
     *
     * @param args "timeout a_value" can be specified to indicate a 
     * timeout value.  If neither "timeout" is specified from the system
     * property nor the command line, the default value will be used.
     */
     ...
     * @param timeout the timeout set by the user
     */
    ...
    /**
     * Sets the transport protocol configured by the user.  If it is null or
     * not one of the supported OBEX transport protocols, the default transport
     * protocol btgoep is used.
     * @param value the transport protocol configured by the user
     */
    private  void setTransport(String value){
    	...
    	if ( value.equals(HelperUtil.BT) ||
    		 value.equals(HelperUtil.TCP) ||
    		 value.equals(HelperUtil.IRDA)) {
    		this.transport = value;
    	} else {
    		System.out.println("Unsupported transport protocol: " + value + 
    				". Default transport " + this.transport + " protocol is used.");
    	}
    }
    ...
    class MyAuthenticator implements Authenticator {
        ...
        /**
         * Creates a <code>MyAuthenticator</code> object with the
         * parent thread specified.
         *
         * @param p the parent of this authenticator thread
         */
        ...
        /**
         * Called each time the server receives an authentication
         * challenge header. It returns with a null user ID and the
         * password of "Password1".
         *
         * @param description ignored
         *
         * @param isUserIdRequired ignored
         *
         * @param isFullAccess ignored
         *
         * @return a <code>PasswordAuthentication</code> object with
         * a <code>null</code> user ID and a password of "Password1"
         */
        public PasswordAuthentication onAuthenticationChallenge(
            ...
            boolean isFullAccess) {
                ...
                return new PasswordAuthentication(null,
                    new String("Password1").getBytes());
        }

        /**
         * Called each time the server receives an authentication
         * response header.  It tracks that the method was called and
         * returns with the "Password" password.
         *
         * @userName ignored
         *
        ...
        public byte[] onAuthenticationResponse(byte[] userName) {
            parent.AUTHENTICATE_TESTED = true;
    ...
    class RequestHandlerImpl extends ServerRequestHandler {
        ...
        /**
         * Creates a <code>RequestHandlerImpl</code> with the parent
         * specified.
         *
         * @param p the parent to this ServerRequestHandler
         */
         ...
         *
         * @param request the headers received in the request
         *
         * @param reply the headers to send in the reply
         *
         * @param backup <code>true</code> if the "back up" flag is
         * set; otherwise <code>false</code>
         *
         * @param create <code>true</code> if the "create" flag is
         * set; otherwise <code>false</code>
         *
        ...
        public int onSetPath(HeaderSet request, HeaderSet reply,
            ...
            if (name.equals("LENGTH")) {
                try {
                    String description = (String)request.getHeader(
                            HeaderSet.DESCRIPTION);
                    if (description == null) {
                        return ResponseCodes.OBEX_HTTP_BAD_REQUEST;
                    }
                    reply.setHeader(HeaderSet.LENGTH,
                        new Long(description.length()));
                    return ResponseCodes.OBEX_HTTP_OK;
                } catch (IOException e) {
        ...
        public void onDisconnect(HeaderSet headers, HeaderSet reply) {
            ...
            if (testName != null) {
                if (testName.equals("HEADER-RESPONSE")) {
                    reply.setHeader(HeaderSet.DESCRIPTION,
                        "Received Request");
                }
            }

            parent.agentthread.resetAgent();
            parent.AUTHENTICATE_TESTED = false;
            return;
        }

        /**
         * Called when the client issues a CONNECT request.  Tests the
         * headers and sends the proper response to the server.
         *
         * @param request the headers received in the request
         *
         * @param reply the headers to send in the reply
        ...
        public int onConnect(HeaderSet headers, HeaderSet reply) {
            ...
            while ((parent.agentthread == null) ||
                (!parent.agentthread.isAlive())) {
                try {
            ...
            if ((testName.equals("TESTAUTHENTICATE"))
                                            && (!AUTHENTICATE_TESTED)) {
                try {
                    reply.createAuthenticationChallenge("Server Auth Test",
                                                            true, true);
                } catch(Exception ex) {
         ...
         *
         * @param tran the <code>Operation</code> object to use with
         * this request
         *
        ...
        public int onGet(Operation tran) {
            ...
            if (testName.equals("ECHO")) {
                try {
                    byte[] message =
                        ((String)hSet.getHeader(HeaderSet.DESCRIPTION)).getBytes();
        ...
        public int onPut(Operation tran) {
            ...
            if (testName.equals("READ")) {
                try {
                    int length = (int)
                        ((Long)hSet.getHeader(HeaderSet.LENGTH)).longValue();
            ...
            if (name != null) {
                ...
                if (name.equals("LENGTH")) {
                	try {
                		...
                		.getHeader(HeaderSet.DESCRIPTION);
                		if (description == null) {
                			return ResponseCodes.OBEX_HTTP_BAD_REQUEST;
                		}
                		reply.setHeader(HeaderSet.LENGTH, new Long(description
                				.length()));
                		return ResponseCodes.OBEX_HTTP_OK;
                	} catch (IOException e) {
                		return ResponseCodes.OBEX_HTTP_INTERNAL_ERROR;
